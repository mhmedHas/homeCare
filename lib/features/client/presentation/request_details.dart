...
class _RequestDetailsScreenState extends State<RequestDetailsScreen> {
  ...
  Future<void> _loadRequest() async {
    ...
    try {
      ...
      if (mounted)
        setState(() {
          _errorMessage = 'تعذر تحميل الطلب';
          _isLoading = false;
        });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(title: const Text('تفاصيل الطلب'), actions: [
          IconButton(onPressed: _loadRequest, icon: const Icon(Icons.refresh))
        ]),
        body: _isLoading
                            ...
                            children: [
                                      ...
                                      child: Column(
                                          ...
                                          children: [
                                            ...
                                            _row('نوع الرعاية',
                                                request.careType),
                                            _row('الشيفت',
                                                '${request.shiftHours} ساعة × ${request.daysCount} يوم'),
                                            _row(
                                                'التاريخ',
                                ...
                                SizedBox(
                                    ...
                                    child: FilledButton.icon(
                                        onPressed: () => context.push(
                                            '/client/request-offers/${request.id}'),
                                        icon: const Icon(Icons.people_outline),
                                        label: Text(_offerCount == 0
                                            ? 'انتظار عروض الممرضين'
                              ...
                              if (_bookingId != null) ...[
                                ...
                                SizedBox(
                                    ...
                                    child: FilledButton.icon(
                                        onPressed: () => context.push(
                                            '/client/booking-details/$_bookingId'),
                                        icon: const Icon(
                                            Icons.calendar_month_outlined),
                                        label: const Text('فتح الحجز'))),
                              ],
                            ]),
                      ));
  }
  ...
  String _statusLabel(String status) {
    switch (status) {
      case 'open':
        return 'الطلب مفتوح لاستقبال عروض الممرضين';
      case 'matching':
        return 'جاري استقبال عروض الممرضين';
      case 'booked':
        return 'تم اختيار ممرض لهذا الطلب';
      case 'in_progress':
        return 'الرعاية جارية';
      case 'completed':
