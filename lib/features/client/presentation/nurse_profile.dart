...
class _NurseProfileScreenState extends State<NurseProfileScreen> {
    ...
    return ListView(
      ...
      children: [
        Card(child: Padding(padding: const EdgeInsets.all(20), child: Column(children: [
          ...
          if (verified) ...[const SizedBox(height: 6), const Row(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.verified, color: AppColors.success, size: 19), SizedBox(width: 5), Text('ممرض موثق', style: TextStyle(color: AppColors.success, fontWeight: FontWeight.w700))])],
          const SizedBox(height: 16),
          Wrap(alignment: WrapAlignment.center, spacing: 8, runSpacing: 8, children: [_stat(Icons.medical_services_outlined, specialization), _stat(Icons.workspace_premium_outlined, '$experience سنوات خبرة')]),
        ]))),
        const SizedBox(height: 12),
        ...
        if (services.isNotEmpty) ...[const SizedBox(height: 12), _section('الخدمات المقدمة', Icons.volunteer_activism_outlined, Wrap(spacing: 8, runSpacing: 8, children: services.map((service) => Chip(label: Text(service))).toList()))],
        const SizedBox(height: 12),
        _reviewsSection(),
  ...
  Widget _reviewsSection() {
    if (_reviews.isEmpty) {
      return Card(child: Padding(padding: const EdgeInsets.all(18), child: Column(children: [const Icon(Icons.rate_review_outlined, size: 40), const SizedBox(height: 8), const Text('لا توجد تقييمات بعد', style: TextStyle(fontWeight: FontWeight.bold)), const SizedBox(height: 4), const Text('ستظهر تقييمات العملاء هنا بعد انتهاء الحجوزات.', textAlign: TextAlign.center, style: TextStyle(color: AppColors.textSecondary))])));
    }
